...
// ds-proxy.execute(dss-proxy-actions, calldata)
...
contract DssProxyActions is Common {
    ...
    function freeGem(address manager, address gemJoin, uint256 cdp, uint256 amt)
        public
    {
    ...
    function exitGem(address manager, address gemJoin, uint256 cdp, uint256 amt)
        public
    {
        // Moves the amount from the CDP urn to proxy's address
        ...
        public
    {
        ...
        if (
            ...
            frob(
                ...
                cdp,
                0,
                _getWipeDart(vat, VatLike(vat).dai(urn), urn, ilk)
            );
        } else {
            // Joins DAI amount into the vat
...
contract DssProxyActionsEnd is Common {
    ...
    function _free(address manager, address end, uint256 cdp)
        internal
        returns (uint256 ink)
    {
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
    ...
    // Public functions
    function freeETH(address manager, address ethJoin, address end, uint256 cdp)
        public
    {
    ...
    function freeGem(address manager, address gemJoin, address end, uint256 cdp)
        public
    {
    ...
    function cashETH(address ethJoin, address end, bytes32 ilk, uint256 wad)
        public
    {
    ...
    function cashGem(address gemJoin, address end, bytes32 ilk, uint256 wad)
        public
    {
        EndLike(end).cash(ilk, wad);
        // Exits token amount to the user's wallet as a token
